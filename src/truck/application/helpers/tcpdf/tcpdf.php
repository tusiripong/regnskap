...
class TCPDF {
			...
			if (isset($format['trans'])) {
				...
				if (isset($format['trans']['S']) && in_array($format['trans']['S'], $stansition_styles)) {
					...
					if (isset($format['trans']['Di']) && in_array($format['trans']['S'], $valid_effect)) {
						if (((($format['trans']['Di'] == 90) OR ($format['trans']['Di'] == 180)) && ($format['trans']['S'] == 'Wipe'))
							OR (($format['trans']['Di'] == 315) && ($format['trans']['S'] == 'Glitter'))
	...
	protected function adjustCellPadding($brd=0) {
		...
		if (is_string($brd)) {
			...
			$brd = $newbrd;
		} elseif (($brd === 1) OR ($brd === true) OR (is_numeric($brd) && (intval($brd) > 0))) {
			$brd = array('LRTB' => true);
		}
		if (!is_array($brd)) {
			return;
	 ...
	 * @param $orientation (string) page orientation. Possible values are (case insensitive):<ul><li>P or PORTRAIT (default)</li><li>L or L&&SCAPE</li></ul>
	 * @param $format (mixed) The format used for pages. It can be either: one of the string values specified at getPageSizeFromFormat() or an array of parameters specified at setPageFormat().
	 * @param $keepmargins (boolean) if true overwrites the default page margins with the current margins
	...
	public function GetCharWidth($char, $notlast=true) {
		...
		if (($this->font_spacing < 0) OR (($this->font_spacing > 0) && $notlast)) {
	...
	public function Annotation($x, $y, $w, $h, $text, $opt=array('Subtype'=>'Text'), $spaces=0) {
		...
		if (!$this->pdfa_mode) {
			if ((($opt['Subtype'] == 'FileAttachment') OR ($opt['Subtype'] == 'Sound')) && (!TCPDF_STATIC::empty_string($opt['FS']))
				&& (@file_exists($opt['FS']) OR TCPDF_STATIC::isValidURL($opt['FS']))
	...
	protected function getCellCode($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=false, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='M') {
		...
		if ($txt != '') {
						...
						for ($i = 0; $i < $numchars; $i++) {
							if (($unicode[$i] >= 0x0e00) && ($unicode[$i] <= 0x0e5b)) {
								...
								if (in_array($ch0, $tonemark)) {
									...
									} elseif (in_array($ch1, $longtail) OR (in_array($ch2, $longtail) && in_array($ch1, $lowvowel))) {
								...
								} elseif (($ch0 == 0x0e33) && (in_array($ch1, $longtail) OR (in_array($ch2, $longtail) && in_array($ch1, $tonemark)))) {
			...
			if ($stretch > 0) {
				...
				if (($ratio < 1) OR (($ratio > 1) && (($stretch % 2) == 0))) {
	...
	public function Write($h, $txt, $link='', $fill=false, $align='', $ln=false, $stretch=0, $firstline=false, $firstblock=false, $maxh=0, $wadj=0, $margin='') {
		...
		if ((!$firstline) && (($chrwidth > $wmax) OR ($maxchwidth > $wmax))) {
		...
		while ($i < $nb) {
			...
			if ($c == 10) { // 10 = "\n" = new line
				...
				if (($l > $wmax) OR (($c == 173) && (($l + $tmp_shy_replacement_width) >= $wmax))) {
					...
					if ($sep == -1) {
						...
						if (($this->rtl && ($this->x <= ($this->w - $this->rMargin - $this->cell_padding['R'] - $margin['R'] - $chrwidth)))
							OR ((!$this->rtl) && ($this->x >= ($this->lMargin + $this->cell_padding['L'] + $margin['L'] + $chrwidth)))) {
	...
	public function Image($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array()) {
		...
		if (in_array($file, $this->imagekeys)) {
			...
			if (strpos($file, '__tcpdf_'.$this->file_id.'_imgmask_') === FALSE) {
				...
				if ((($oldsize < $newsize) && ($resize)) OR (($oldsize < $pixsize) && (!$resize))) {
					$newimage = true;
				}
			}
		...
		if ($newimage) {
			...
			$info = false;
			if ((method_exists('TCPDF_IMAGES', $mtd)) && (!($resize && (function_exists($gdfunction) OR extension_loaded('imagick'))))) {
				// TCPDF image functions
				$info = TCPDF_IMAGES::$mtd($file);
				if (($ismask === false) && ($imgmask === false) && (strpos($file, '__tcpdf_'.$this->file_id.'_imgmask_') === FALSE)
					&& (($info === 'pngalpha') OR (isset($info['trns']) && !empty($info['trns'])))) {
	...
	public function _destroy($destroyall=false, $preserve_objcopy=false) {
		...
		foreach (array_keys(get_object_vars($this)) as $val) {
			if ($destroyall OR !in_array($val, $preserve)) {
				if ((!$preserve_objcopy OR ($val != 'objcopy')) && ($val != 'file_id') && isset($this->$val)) {
					unset($this->$val);
				}
			}
		}
	...
	protected function _getannotsrefs($n) {
		if (!(isset($this->PageAnnots[$n]) OR ($this->sign && isset($this->signature_data['cert_type'])))) {
			return '';
		}
		$out = ' /Annots [';
	...
	protected function _putfonts() {
		...
		foreach ($this->FontFiles as $file => $info) {
			...
			if (!TCPDF_STATIC::empty_string($fontfile)) {
				...
				} elseif ($info['subset'] && ((!$compressed) OR ($compressed && function_exists('gzcompress')))) {
					if ($compressed) {
						// uncompress font
	...
	protected function _putcatalog() {
		...
		if (!empty($this->form_obj_id)
			OR ($this->sign && isset($this->signature_data['cert_type']))
			...
			// signatures
			if ($this->sign && isset($this->signature_data['cert_type']) 
				&& (empty($this->signature_data['approval']) OR ($this->signature_data['approval'] != 'A'))) {
				if ($this->signature_data['cert_type'] > 0) {
	...
	public function RoundedRectXY($x, $y, $w, $h, $rx, $ry, $round_corner='1111', $style='', $border_style=array(), $fill_color=array()) {
		...
		if (($round_corner == '0000') OR (($rx == $ry) && ($rx == 0))) {
			// Not rounded
	...
	protected function _putjavascript() {
		if ($this->pdfa_mode OR (empty($this->javascript) && empty($this->js_objects))) {
			return;
		}
				...
				switch ($cmd) {
					...
					case 'L': {
						...
						foreach ($chunks as $key => $val) {
							if ((($key % 2) == 0) && (($val < $x1) OR ($val > $x2))) {
								$skip = true;
							} elseif ((($key % 2) != 0) && (($val < $y1) OR ($val > $y2))) {
								$skip = true;
							}
						}
					}
	...
	public function write1DBarcode($code, $type, $x='', $y='', $w='', $h='', $xres='', $style='', $align='') {
		...
		if ($style['stretch']) {
			...
			if ((isset($style['padding']) && ($style['padding'] === 'auto'))
				OR (isset($style['hpadding']) && ($style['hpadding'] === 'auto'))) {
				$hpadding = 10 * $xres;
		...
		if (preg_match_all('/<link([^\>]*)>/isU', $html, $matches) > 0) {
			foreach ($matches[1] as $key => $link) {
				...
				if (preg_match('/type[\s]*=[\s]*"text\/css"/', $link, $type)) {
					...
					if (empty($type) OR (isset($type[1]) && (($type[1] == 'all') OR ($type[1] == 'print')))) {
						$type = array();
			...
			if (preg_match($tagpattern, $element)) {
				...
				if ($element[0] == '/') {
					...
					if ((($dom[$key]['value'] == 'ul') OR ($dom[$key]['value'] == 'ol') OR ($dom[$key]['value'] == 'dl'))
						&& (!isset($dom[$key]['align']) OR TCPDF_STATIC::empty_string($dom[$key]['align']) OR ($dom[$key]['align'] != 'J'))) {
						if ($this->rtl) {
							$dom[$key]['align'] = 'R';
						} else {
							$dom[$key]['align'] = 'L';
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	public function writeHTML($html, $ln=true, $fill=false, $reseth=false, $cell=false, $align='') {
		...
		while ($key < $maxel) {
			if ($dom[$key]['tag'] && $dom[$key]['opening'] && $dom[$key]['hide']) {
				...
				if ($dom[$key]['self']) {
					...
					while (($key < $maxel) && (!$dom[$key]['tag'] OR $dom[$key]['opening'] OR ($dom[$key]['parent'] != $hidden_node_key))) {
						// skip hidden objects
						++$key;
					}
					++$key;
			...
			if (!empty($dom[$key]['tag']) && isset($dom[$key]['attribute']['pagebreak'])) {
				// check for pagebreak
				...
				if ((($dom[$key]['attribute']['pagebreak'] == 'left') && (((!$this->rtl) && (($this->page % 2) == 0)) OR (($this->rtl) && (($this->page % 2) != 0))))
					OR (($dom[$key]['attribute']['pagebreak'] == 'right') && (((!$this->rtl) && (($this->page % 2) != 0)) OR (($this->rtl) && (($this->page % 2) == 0))))) {
					// add a page (or trig AcceptPageBreak() for multicolumn mode)
					$this->checkPageBreak($this->PageBreakTrigger + 1);
					$this->htmlvspace = ($this->PageBreakTrigger + 1);
				}
			}
			if (!empty($dom[$key]['tag']) && !empty($dom[$key]['opening']) && isset($dom[$key]['attribute']['nobr']) && ($dom[$key]['attribute']['nobr'] == 'true')) {
			...
			// print THEAD block
			if ((!empty($dom[$key]['value']) && $dom[$key]['value'] == 'tr') && isset($dom[$key]['thead']) && !empty($dom[$key]['thead'])) {
				...
				while ( ($key < $maxel) && (!(
					($dom[$key]['tag'] && $dom[$key]['opening'] && ($dom[$key]['value'] == 'tr') && (!isset($dom[$key]['thead']) OR !$dom[$key]['thead']))
					OR ($dom[$key]['tag'] && (!$dom[$key]['opening']) && ($dom[$key]['value'] == 'table'))) )) {
					++$key;
				}
			}
			if (!empty($dom[$key]['tag']) OR ($key == 0)) {
				if ((($dom[$key]['value'] == 'table') OR ($dom[$key]['value'] == 'tr')) && (isset($dom[$key]['align']))) {
				...
				if ((!$this->newline) && ($dom[$key]['value'] == 'img') && (isset($dom[$key]['height'])) && ($dom[$key]['height'] > 0)) {
					...
					if (!empty($dom[$key]['width'])) {
						...
						if (($imgw <= ($this->w - $this->lMargin - $this->rMargin - $this->cell_padding['L'] - $this->cell_padding['R']))
							...
							OR ((!$this->rtl) && (($this->x + $imgw) > ($this->w - $this->rMargin - $this->cell_padding['R']))))) {
					...
					if (($fontname != $curfontname) OR ($fontstyle != $curfontstyle) OR ($fontsize != $curfontsize)
						...
						OR ($dom[$key]['tag'] && $dom[$key]['opening'] && ($dom[$key]['value'] == 'li')) ) {
						if (($key < ($maxel - 1)) && (
								($dom[$key]['tag'] && $dom[$key]['opening'] && ($dom[$key]['value'] == 'li'))
								OR ($this->cell_height_ratio != $dom[$key]['line-height'])
								OR (!$this->newline && is_numeric($fontsize) && is_numeric($curfontsize)
								&& ($fontsize >= 0) && ($curfontsize >= 0)
								&& (($fontsize != $curfontsize) OR ($fontstyle != $curfontstyle) OR ($fontname != $curfontname)))
							)) {
							if ($this->page > $startlinepage) {
							...
							if (!$dom[$key]['block']) {
								...
								if (($dom[$key]['value'] != 'sup') && ($dom[$key]['value'] != 'sub')) {
									...
									if (isset($line_align_data) && (($line_align_data[0] == ($key - 1)) OR (($line_align_data[0] == ($key - 2)) && (isset($dom[($key - 1)])) && (preg_match('/^([\s]+)$/', $dom[($key - 1)]['value']) > 0)))) {
			...
			// align lines
			if (!empty($dom[$key]['value']) && $this->newline && (strlen($dom[$key]['value']) > 0) && ($dom[$key]['value'] != 'td') && ($dom[$key]['value'] != 'th')) {
				$newline = true;
				$fontaligned = false;
				...
				if (isset($startlinex)) {
					...
					if ((isset($plalign) && ((($plalign == 'C') OR ($plalign == 'J') OR (($plalign == 'R') && (!$this->rtl)) OR (($plalign == 'L') && ($this->rtl)))))) {
			...
			if (!empty($dom[$key]['tag'])) {
						...
						if (isset($dom[$table_el]['rowspans'])) {
							...
							while ($rsk < $rskmax) {
								...
								if (($trwsp['rowspan'] > 0)
									...
									&& (($trwsp['starty'] < ($this->y - $this->feps)) OR ($trwsp['startpage'] < $this->page) OR ($trwsp['startcolumn'] < $this->current_column))) {
			...
			} elseif (!empty($dom[$key]['value']) && strlen($dom[$key]['value']) > 0) {
				// print list-item
				...
				if (!isset($dom[$key]['trimmed_space']) OR !$dom[$key]['trimmed_space']) {
					...
					if (!empty($this->HREF) && (isset($this->HREF['url']))) {
						...
						if (($wadj > 0) && (($this->rtl && ($this->tmprtl === 'L')) OR (!$this->rtl && ($this->tmprtl === 'R')))) {
			...
			++$key;
			if (isset($dom[$key]['tag']) && $dom[$key]['tag'] && (!isset($dom[$key]['opening']) OR !$dom[$key]['opening']) && isset($dom[($dom[$key]['parent'])]['attribute']['nobr']) && ($dom[($dom[$key]['parent'])]['attribute']['nobr'] == 'true')) {
				// check if we are on a new page or on a new column
				if ((!$undo) && (($this->y < $this->start_transaction_y) OR (($dom[$key]['value'] == 'tr') && ($dom[($dom[$key]['parent'])]['endy'] < $this->start_transaction_y)))) {
					// we are on a new page or on a new column and the total object height is less than the available vertical space.
					// restore previous object
					$this->rollbackTransaction(true);
					// restore previous values
		...
		if (isset($startlinex)) {
			...
			if ((isset($plalign) && ((($plalign == 'C') OR (($plalign == 'R') && (!$this->rtl)) OR (($plalign == 'L') && ($this->rtl)))))) {
				// calculate shifting amount
				$tw = $w;
				if ($this->lMargin != $prevlMargin) {
					$tw += ($prevlMargin - $this->lMargin);
		...
		if ($ln && (!($cell && (!empty($dom[$key-1]['value']) && $dom[$key-1]['value'] == 'table')))) {
			$this->Ln($this->lasth);
	...
	protected function openHTMLTagHandler($dom, $key, $cell) {
		...
		switch($tag['value']) {
			...
			case 'input': {
				...
				if (isset($tag['attribute']['checked']) && (($tag['attribute']['checked'] == 'checked') OR ($tag['attribute']['checked'] == 'true'))) {
					$checked = true;
				} else {
					$checked = false;
				}
		...
		if ($dom[$key]['self'] && isset($dom[$key]['attribute']['pagebreakafter'])) {
			...
			if ((($pba == 'left') && (((!$this->rtl) && (($this->page % 2) == 0)) OR (($this->rtl) && (($this->page % 2) != 0))))
				OR (($pba == 'right') && (((!$this->rtl) && (($this->page % 2) != 0)) OR (($this->rtl) && (($this->page % 2) == 0))))) {
				// add a page (or trig AcceptPageBreak() for multicolumn mode)
				$this->checkPageBreak($this->PageBreakTrigger + 1);
			}
		}
	...
	protected function closeHTMLTagHandler($dom, $key, $cell, $maxbottomliney=0) {
		$tag = $dom[$key];
		$parent = $dom[($dom[$key]['parent'])];
		$lasttag = ((!isset($dom[($key + 1)])) OR ((!isset($dom[($key + 2)])) && ($dom[($key + 1)]['value'] == 'marker')));
		$in_table_head = false;
	...
	public function selectColumn($col='') {
		...
		if ($this->num_columns > 1) {
			if ($col != $this->current_column) {
				...
				if (($this->page > $this->maxselcol['page']) OR (($this->page == $this->maxselcol['page']) && ($col > $this->maxselcol['column']))) {
					$enable_thead = true;
	...
	*/
	public function setTextShadow($params=array('enabled'=>false, 'depth_w'=>0, 'depth_h'=>0, 'color'=>false, 'opacity'=>1, 'blend_mode'=>'Normal')) {
		...
		if ((($this->txtshadow['depth_w'] == 0) && ($this->txtshadow['depth_h'] == 0)) OR ($this->txtshadow['opacity'] == 0)) {
			$this->txtshadow['enabled'] = false;
		}
	}
	...
	public function addPageRegion($region) {
		...
		if (isset($region['xt']) && isset($region['xb']) && ($region['xt'] > 0) && ($region['xb'] > 0)
			...
			&& isset($region['side']) && (($region['side'] == 'L') OR ($region['side'] == 'R'))) {
	...
	protected function setSVGStyles($svgstyle, $prevsvgstyle, $x=0, $y=0, $w=1, $h=1, $clip_function='', $clip_params=array()) {
		...
		if (preg_match('/url\([\s]*\#([^\)]*)\)/si', $svgstyle['fill'], $regs)) {
			...
			if (!empty($clip_function) && method_exists($this, $clip_function)) {
				...
				if ((!isset($gradient['type']) OR ($gradient['type'] != 3)) && is_array($bbox) && (count($bbox) == 4)) {
					list($x, $y, $w, $h) = $bbox;
				}
			}
			...
			switch (strtoupper($cmd)) {
				...
				case 'S': { // shorthand/smooth curveto
					foreach ($params as $ck => $cp) {
						$params[$ck] = $cp;
						if ((($ck + 1) % 4) == 0) {
							if (($key > 0) && ((strtoupper($paths[($key - 1)][1]) == 'C') OR (strtoupper($paths[($key - 1)][1]) == 'S'))) {
								$x1 = (2 * $x) - $x2;
								$y1 = (2 * $y) - $y2;
							} else {
								$x1 = $x;
				...
				case 'T': { // shorthand/smooth quadratic Bezier curveto
					foreach ($params as $ck => $cp) {
						$params[$ck] = $cp;
						if (($ck % 2) != 0) {
							if (($key > 0) && ((strtoupper($paths[($key - 1)][1]) == 'Q') OR (strtoupper($paths[($key - 1)][1]) == 'T'))) {
								$x1 = (2 * $x) - $x1;
								$y1 = (2 * $y) - $y1;
							} else {
								$x1 = $x;
					...
					foreach ($params as $ck => $cp) {
						...
						if ((($ck + 1) % 7) == 0) {
							...
							if ((abs($x0 - $x) < $minlen) && (abs($y0 - $y) < $minlen)) {
								...
								$angf = $angs - $dang;
								if ((($fs == 0) && ($angs > $angf)) OR (($fs == 1) && ($angs < $angf))) {
									// reverse angles
									$tmp = $angs;
									$angs = $angf;
									$angf = $tmp;
	...
	protected function startSVGElementHandler($parser, $name, $attribs, $ctm=array()) {
		...
		if ($clipping && !isset($attribs['fill']) && (!isset($attribs['style']) OR (!preg_match('/[;\"\s]{1}fill[\s]*:[\s]*([^;\"]*)/si', $attribs['style'], $attrval)))) {
		...
		switch($name) {
			...
			case 'svg': {
				...
				if (isset($attribs['viewBox'])) {
					...
					if (sizeof($tmp) == 4) {
						...
						$hr = ($svgH / $vh);
						$ax = $ay = 0;
						if ((($fit == 'meet') && ($hr < $wr)) OR (($fit == 'slice') && ($hr > $wr))) {
							if ($aspectX == 'xMax') {
								$ax = (($vw * ($wr / $hr)) - $vw);
							}
							if ($aspectX == 'xMid') {
							...
							$wr = $hr;
						} elseif ((($fit == 'meet') && ($hr > $wr)) OR (($fit == 'slice') && ($hr < $wr))) {
							if ($aspectY == 'YMax') {
								$ay = (($vh * ($hr / $wr)) - $vh);
							}
							if ($aspectY == 'YMid') {
			...
			case 'linearGradient': {
				...
				if (((!isset($attribs['x1'])) && (!isset($attribs['y1'])) && (!isset($attribs['x2'])) && (!isset($attribs['y2'])))
					OR ((isset($attribs['x1']) && (substr($attribs['x1'], -1) == '%'))
						OR (isset($attribs['y1']) && (substr($attribs['y1'], -1) == '%'))
						OR (isset($attribs['x2']) && (substr($attribs['x2'], -1) == '%'))
						OR (isset($attribs['y2']) && (substr($attribs['y2'], -1) == '%')))) {
			...
			case 'radialGradient': {
				...
				if (((!isset($attribs['cx'])) && (!isset($attribs['cy'])))
					OR ((isset($attribs['cx']) && (substr($attribs['cx'], -1) == '%'))
					OR (isset($attribs['cy']) && (substr($attribs['cy'], -1) == '%')))) {
					$this->svggradients[$this->svggradientid]['mode'] = 'percentage';
			...
			case 'image': {
				...
				if (!$clipping) {
					...
					if (preg_match('/^data:image\/[^;]+;base64,/', $img, $m) > 0) {
						...
						// fix image path
						if (!TCPDF_STATIC::empty_string($this->svgdir) && (($img[0] == '.') OR (basename($img) == $img))) {
